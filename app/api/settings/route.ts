...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('user_settings')
    .select('scrape_api_key, groq_api_key, is_onboarded, keywords, city, radius, last_scrape_at')
    .eq('user_id', user.id)
    .single();
...
export async function DELETE(request: Request) {
  ...
  const { error } = await supabase.from('user_settings').update({ scrape_api_key: null, updated_at: new Date().toISOString() }).eq('user_id', user.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}
