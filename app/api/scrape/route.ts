...
async function runScrapeForUser(userId: string, supabase: any, customTags: string[], skipCooldown: boolean) {
  ...
  if (!skipCooldown) {
    ...
    if (cooldownRejected) {
      ...
      return { success: false, error: `Scrape cooldown active. Retry in ${retryAfter}s.`, status: 429 };
    }
  }
  ...
  const activeKeywords: string[] =
    customTags.length > 0
      ...
      ? userKeywords.slice(0, 10)
      : TITLE_KEYWORDS;

  await slog.info('scrape', 'Scrape gestart', { city: userCity, radius: userRadius, keywords: activeKeywords.length }, userId);
  ...
  if (adzunaId && adzunaKey) {
    ...
    for (let i = 0; i < activeKeywords.length; i += BATCH) {
      ...
      for (const result of results) {
        if (result.status === 'rejected') {
          await slog.warn('scrape', 'Adzuna fetch mislukt', { reason: String(result.reason) }, userId);
          continue;
        }
    ...
    await slog.warn('scrape', 'Geen Adzuna API-sleutels ingesteld', {}, userId);
  }
  ...
  return { success: true, count: data?.length ?? 0, total_found: uniqueJobs.length };
}
