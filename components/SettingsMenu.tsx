    ...
    <div className="flex flex-col gap-3 rounded-2xl p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      <div>
        ...
        <p className="text-xs" style={{ color: 'var(--text2)' }}>
          Voor AI-scoring &amp; motivatiebrieven. Gratis op{' '}
          <a href="https://console.groq.com" target="_blank" rel="noreferrer" className="underline" style={{ color: '#7c3aed' }}>console.groq.com</a>
        </p>
      </div>
      {key ? (
...
function AdzunaSection({ initial }: { initial: { id: string | null; key: string | null; today: number; month: number } }) {
  ...
  const save = async () => {
    if (!idInput.trim() || !keyInput.trim()) return;
    setLoading(true);
    ...
    setLoading(false);
    if (d.success) {
      setIdVal(`${idInput.slice(0, 4)}...${idInput.slice(-4)}`);
      setKeyVal(`${keyInput.slice(0, 4)}...${keyInput.slice(-4)}`);
      setIdInput(''); setKeyInput('');
      flash('Opgeslagen');
    }
  };
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <div className="rounded-xl p-3 flex flex-col gap-2.5" style={{ background: 'var(--surface2)' }}>
        <div className="flex flex-col gap-1">
          <div className="flex justify-between">
            <span className="text-xs" style={{ color: 'var(--text2)' }}>Vandaag</span>
            <span className="text-xs font-mono" style={{ color: initial.today >= 200 ? 'var(--red)' : 'var(--text2)' }}>{initial.today} / 250</span>
          </div>
          <UsageBar value={initial.today} max={250} color="#6366f1" />
        </div>
        <div className="flex flex-col gap-1">
          <div className="flex justify-between">
            <span className="text-xs" style={{ color: 'var(--text2)' }}>Deze maand</span>
            <span className="text-xs font-mono" style={{ color: initial.month >= 2000 ? 'var(--red)' : 'var(--text2)' }}>{initial.month} / 2500</span>
          </div>
      ...
      {idVal && keyVal ? (
        ...
        <p className="text-xs" style={{ color: 'var(--text2)' }}>
          Gratis via{' '}
          <a href="https://developer.adzuna.com" target="_blank" rel="noreferrer" className="underline" style={{ color: '#6366f1' }}>developer.adzuna.com</a>
        </p>
      )}
...
function KeywordsSection({ initial }: { initial: string[] }) {
  ...
  const add = () => {
    ...
    setKeywords(next); setInput(''); persist(next);
  };

  const remove = (kw: string) => {
    const next = keywords.filter(k => k !== kw);
    setKeywords(next); persist(next);
  };
...
function DangerSection() {
  ...
  const deleteAll = async () => {
    ...
    setLoading(false); setDone(true); setConfirm(false);
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl p-4" style={{ background: 'var(--surface)', border: '1px solid rgba(248,113,113,0.2)' }}>
      ...
      {!confirm ? (
        ...
        <div className="flex gap-2">
          <button onClick={() => setConfirm(false)} className="flex-1 text-sm py-2 rounded-xl"
            style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text2)' }}>Annuleer</button>
          <button onClick={deleteAll} disabled={loading} className="flex-1 text-sm py-2 rounded-xl font-medium disabled:opacity-40"
            style={{ background: 'var(--red)', color: '#fff' }}>{loading ? '...' : 'Ja, verwijder'}</button>
        </div>
      )}
    </div>
  );
...
export default function SettingsMenu() {
  ...
  useEffect(() => {
    fetch('/api/settings').then(r => r.json()).then(d => {
      setData(d);
      if (d.keywords?.length) localStorage.setItem('ja_tags', JSON.stringify(d.keywords));
    });
  }, []);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between gap-3 rounded-2xl p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs" style={{ color: 'var(--text2)' }}>
              {data.last_scrape_at ? `Laatste scrape: ${new Date(data.last_scrape_at).toLocaleString('nl-BE')}` : 'Nog niet gescrapet'}
            </p>
          </div>
        </div>
      ...
      {data.is_admin && (
        <AdzunaSection initial={{ id: data.adzuna_app_id, key: data.adzuna_app_key, today: data.adzuna_calls_today ?? 0, month: data.adzuna_calls_month ?? 0 }} />
      )}
