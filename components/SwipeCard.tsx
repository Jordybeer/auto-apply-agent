  ...
  return (
    ...
    >
      ...
      <div className="glass rounded-3xl w-full h-full flex flex-col overflow-hidden shadow-2xl">
        ...
        {/* Header band */}
        <div className="px-6 pt-7 pb-5 flex flex-col gap-1">
        ...
        <div className="flex-1 px-6 overflow-y-auto">
          <p className="text-sm text-[var(--text2)] leading-relaxed line-clamp-6">
        ...
        <div className="px-6 pb-6 pt-4">
          {jobs?.url ? (
            ...
            <div className="w-full py-3 rounded-2xl text-center text-sm text-[var(--text2)] bg-white/5">No URL available</div>
          )}
        </div>
      </div>
      ...
      {/* Swipe hint overlays */}
      <div
        className="absolute top-8 left-8 pointer-events-none transition-opacity duration-150"
        style={{ opacity: hint === 'left' ? 1 : 0 }}
      >
        <div className="rounded-2xl border-2 border-[var(--red)] px-4 py-2">
          <Lottie lottieRef={crossRef} animationData={crossData} loop={false} autoplay={false} style={{ width: 44, height: 44 }} />
        </div>
      </div>
      <div
        className="absolute top-8 right-8 pointer-events-none transition-opacity duration-150"
        style={{ opacity: hint === 'right' ? 1 : 0 }}
      >
        <div className="rounded-2xl border-2 border-[var(--green)] px-4 py-2">
          <Lottie lottieRef={checkRef} animationData={checkData} loop={false} autoplay={false} style={{ width: 44, height: 44 }} />
        </div>
      </div>
    </div>
  );
