      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button type="button" onClick={() => addSkill(newSkill)} disabled={!newSkill.trim()}
            style={{ background: newSkill.trim() ? 'var(--accent)' : 'var(--surface2)',
              color: newSkill.trim() ? '#fff' : 'var(--text2)',
              border: 'none', borderRadius: 10, padding: '0 14px', cursor: 'pointer',
              display: 'flex', alignItems: 'center', gap: 4, height: 44, flexShrink: 0, fontSize: 13, fontWeight: 600 }}>
            <Plus size={14} /> Voeg toe
          </button>
        ...
        <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 0 }}>
          ...
          <button type="button"
            ...
            style={{ width: 44, height: 26, borderRadius: 999, border: 'none', cursor: 'pointer',
              background: value.open_to_retraining ? 'var(--accent)' : 'var(--surface3)',
              transition: 'background 0.2s', position: 'relative', flexShrink: 0 }}
            aria-pressed={value.open_to_retraining}>
            <span style={{ position: 'absolute', top: 3, left: value.open_to_retraining ? 21 : 3,
